	...
	pjax.request = function(options){
		...
		options = mix({
			...
			onsucceed: function(data, isCache){
				...
				if(isFunction(options._callback)){
...
(function(){
	var endPjaxTimer = 0;
	W('#content').on('start.pjax', function(cache){
		W('.loading').show();
		endPjaxTimer = setTimeout(function(){
			endPjaxTimer = 0;
			W('#content').fire('end.pjax');
		}, 3000);
	});
	var li = W('header nav li');
	W('#content').on('end.pjax', function(cache){
		if(endPjaxTimer){
		...
		li.forEach(function(el){
			...
			href = QW.pjax.getRealUrl(href);
			h = QW.pjax.getRealUrl(location.href);
