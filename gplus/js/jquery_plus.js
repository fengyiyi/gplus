...
(function($){
	...
	function getCache(src, time, flag){
		...
		if(src in pjaxStack){
			...
			if(item){
				tval = parseInt(window.localStorage.getItem(tkey));
	...
	$.pjax = function(options){
		...
		cache = parseInt($(options.element).attr('data-pjax-cache')); //各个链接里可以加自己的缓存时间，主要是应对不同的类型进行不同的缓存
...
$(function(){
	...
	var endPjaxTimer = 0;
	$('#content').bind('start.pjax', function(cache){
		$('.loading').show();
		endPjaxTimer = setTimeout(function(){
			endPjaxTimer = 0;
			$('#content').trigger('end.pjax');
		}, 3000);
	});
	var li = $('header nav li');
	$('#content').bind('end.pjax', function(cache){
		if(endPjaxTimer){
	...
	var a = $('a[href^="'+pjaxHomeUrl+'"]');
	a.pjax('#content',{
		fx: typeof pjaxFx == 'undefined' ? '' : pjaxFx,
		...
		filter: function(){
			var a = $(this).not('[href*="/wp-content/"]').not('[href*="/wp-admin"]');
